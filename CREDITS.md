...
太阳日面**不再使用任何图片文件**（原先的 `textures/sun-albedo.jpg` 已删除），改为运行时程序化生成，原因见下。

### 太阳日面为什么撤掉了 NASA 图片

原先用的是 NASA / SDO 的 HMI 连续谱强度图 `latest_2048_HMIIC.jpg`：单张圆盘图只覆盖约 180°，
处理办法是先按临边昏暗反演亮度（中心增益 1.00 → 边缘 2.40，7 档滑动平均，整体归一到 176），
再**镜像拼接**成 360° 等距圆柱贴图。

但这么拼有三个绕不过去的毛病：

- 左右两半是同一张图镜像出来的，球一转就能看到**两个重复的亮斑**；
- 中间接缝怎么调都留着一条**竖着的痕迹**；
- 反演过程做了平滑，**米粒组织（granulation）几乎被抹平**，只剩一片均匀的橙色。

结果贴到球上就是一颗「橙色塑料球」，怎么改着色器都不像太阳。

现在日面由 `js/sun-moon.js` 的 `makeSunMap()` 现场生成（512×256 画布，CPU 端算完再交给 GPU）：

- 米粒组织用**球面 Voronoi 的四层细胞场**叠加，经度方向天然无缝、两极也不变形；
- 黑子按真实结构做**本影 + 半影**两层，成对成群压在中低纬度；
- 顺带省掉一张 34KB 的图片，也不再需要 file:// 下的跨域兜底。

必须说明的取舍：真实米粒只有太阳直径的 1/1000，照真实比例画在这张 512 宽的贴图上不到 1px，
所以米粒是**艺术化放大**过的 —— 但只放大成「高频、低对比的细密沙粒感」，而不是大块细胞
（放大成大块会变成龟裂 / 橘子皮，反而更假）。黑子同理：真实本影 0.8~4°，这里放到 2~5° 才看得见。

- 月球 **凹凸/高度图不额外下载**：浏览器加载完反照率贴图后，用它的亮度现场派生（亮 = 高地，暗 = 月海，再做一次 3×3 模糊去掉噪点），所以不会再多出几百 KB 的贴图文件。
- `js/textures-inline.js` 是月亮贴图的 **base64 内联副本**（1024×512）。只有在你**直接双击 index.html**（file:// 协议，浏览器会拦下本地跨域图片）时才会被自动加载，用来兜底；用本地服务器打开时不会加载它，走的是 `textures/` 里的高清原图。太阳不需要副本 —— 它的日面是现场生成的。
- NASA 的图片素材一般是公有领域（public domain），可以自由使用与再分发；Solar System Scope 那一版（CC BY 4.0）因为站点返回 403，最终没有采用。
- 页面里卡片上标注的「NASA LRO 月面反照率图」是给月亮贴图的署名；太阳那一侧写的是「程序化日面（米粒组织 + 黑子）」，因为它的日面已经不再来自任何图片素材。
...
- 站点目前**没有任何音乐功能**：原先的「首页音乐卡」和「歌库」页已整体删除，
  相关代码（`js/library.js`、`audio/library.js`、`audio/overrides.json`，以及
  `modules.js` / `script.js` / `router.js` / `mood.js` / `style.css` 里的对应部分）也一并清掉了。
- `audio/` 目录里只放**本机自用**的 mp3：`.gitignore` 忽略了音频格式，它们不会进仓库、
  也不会出现在公开站点上。若想重新加回歌库功能，步骤见 `audio/README.txt`。
- **版权**：商业歌曲的完整音频不适合放到公开站点，属于侵权风险；
  另外音频体积大，GitHub Pages 建议仓库 ≤ 1GB、月流量 ≤ 100GB。

## 怎么打开

1. **本地服务器（推荐，月亮贴图用 2048×1024 高清原图）**：在 `personal-homepage` 目录下起个静态服务，
   例如 `python -m http.server 5210` 或 `npx serve -l 5210`，然后访问 `http://127.0.0.1:5210/`。
2. **直接双击 `index.html`**：页面功能同样完整，月亮会自动改用 `js/textures-inline.js` 里的
   内联贴图（1024×512），画质略低一点但不会退化回程序化表面；太阳本来就是现场生成的，两种打开方式没差别。
